...
SEA deployment changes pnpm's recorded dependency mode. The Windows workflow keeps the complete installation intact while electron-builder collects production modules instead of allowing pnpm to prune the builder itself. The repository hook installer also checks CI before importing its development-only Lefthook package, so a production-only reconciliation elsewhere can omit development dependencies without weakening normal local hook installation.
