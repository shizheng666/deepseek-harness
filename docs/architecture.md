...
The Windows desktop executable is a deployment shell rather than another Harness application launcher. It starts its packaged normal `dsh` executable as `dsh web --no-open --port 0 --supervised`, then loads the authenticated loopback URL announced by that process.
